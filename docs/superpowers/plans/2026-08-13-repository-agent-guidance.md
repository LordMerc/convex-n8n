...
**Goal:** Add concise repository-wide agent guidance that accurately describes this package and protects its architectural and security boundaries.

**Architecture:** A root `AGENTS.md` will summarize the two-node package, map each source area to its responsibility, state non-negotiable request and credential invariants, and map change types to focused validation. The existing metadata test will treat the guidance as a required repository document.

**Tech Stack:** Markdown, Node.js 22.22.0+, TypeScript 5.9, n8n community-node tooling, Node's built-in test runner.

## Global Constraints

- Keep `AGENTS.md` concise, repository-wide, and focused on agent behavior.
- Preserve the isolation between `convexApi` application credentials and `convexTeamApi` Team Access Tokens.
- Preserve redirect blocking, same-origin HTTP Action confinement, JSON-only handling, secret-safe errors, and per-execution pagination state restoration.
- Do not add dependencies or change runtime behavior.
- Do not commit, push, tag, publish, or modify external services without explicit user authorization.
...
**Interfaces:**
- Consumes: the current directory layout, package scripts, n8n registrations in `package.json`, and implemented security invariants.
- Produces: repository-wide instructions automatically discovered by coding agents and a metadata regression assertion that keeps the file present.

- [ ] **Step 1: Extend the required-document regression test**

Add `'AGENTS.md',` immediately before `'README.md',` in the required-file array in `tests/package-metadata.test.js`.

- [ ] **Step 2: Run the focused test and confirm it fails**

Run: `node --test tests/package-metadata.test.js`

Expected: FAIL with `AGENTS.md should exist` because the guidance has not been created yet.

- [ ] **Step 3: Create the repository guidance**
...
Expected: PASS with both metadata tests passing.

- [ ] **Step 5: Review and stage without committing**

Run: `Get-Content -Raw AGENTS.md`

Run: `git diff -- tests/package-metadata.test.js`

Confirm the architecture map matches current paths, no placeholder or sensitive value exists, and the only test change is the required-file entry. Stage `AGENTS.md`, `tests/package-metadata.test.js`, and this plan/spec documentation. Do not commit because the user has not authorized a commit.

- [ ] **Step 6: Check the staged result**

Run: `git diff --cached --check`

Expected: exit 0 with no whitespace errors.

Run: `git diff --cached -- AGENTS.md tests/package-metadata.test.js`

Confirm the staged guidance and regression assertion match the approved spec. Leave the repository staged but uncommitted.
